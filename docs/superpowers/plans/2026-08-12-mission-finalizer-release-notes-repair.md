...
- Preserve source `cee287597024b5a1e990c6e272518236bc9e32fa`, release ID `369467111`, tag `v0.1.4`, and the exact three public assets.
...
- [x] Add assertions for exact notes digest validation and retention.
- [x] Add assertions for `--notes-file` on new release creation.
- [x] Add assertions for the explicit repair input, distinct confirmation, tag/source/title/body/release-state checks, exact asset digest checks, and metadata-only edit.
- [x] Run `go test ./internal/mission -run TestReleaseFinalizationImportsExactRehearsalArtifacts -count=1` and observe failure.
...
- [x] Validate and retain the exact sealed `release-notes.md`.
- [x] Publish new releases with `--notes-file`.
- [x] Add `repair_empty_release_notes` and its distinct confirmation.
- [x] In repair mode, independently verify the existing public release and exact assets, then edit only its notes.
- [x] Run the focused test and all repository-required local gates.

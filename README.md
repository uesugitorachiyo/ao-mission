...
ao-mission mission dashboard --mission <id> [--compact] [--out <dashboard.json>] [--json]
...
`ao-mission continue` persists `ao.mission.event-loop-decision.v0.1` after each continuation step so the zero-wait event loop has durable no-authority readback. With `--until-done`, v0.3 records a goal lease, checkpoint/resume bundle, return gate, and route reconciliation instead of stopping after the first governed handoff. The default long-run lease is `min_nodes=10`, `min_minutes=120`, `max_minutes=180`, `max_iterations=<command value>`, `return_only_when=mission_done_or_true_hard_blocker_or_no_ready_work_and_no_exact_next_action`, and `checkpoint_policy=after_each_node_or_timed_interval`. `ao-mission next` appends `ao.mission.route-decision.v0.1` entries to the mission route history, `ao-mission mission history` exports that history for AO Command or Atlas inspection, `ao-mission mission archive` writes a digest-bound public-safe archive bundle, `ao-mission mission validate-archive` verifies the archive digest and authority flags, `ao-mission mission import-archive` restores the mission record as readback-only state, and `ao-mission mission compact` trims retained route/step history while recording `ao.mission.ledger-compaction-readback.v0.1` evidence without repository mutation. Use `ao-mission mission compact --dry-run` to preview retained route/step counts without modifying the mission record, and use `ao-mission mission compact --timeline` to emit `ao.mission.timeline-compaction-readback.v0.1` with a digest of retained timeline state for Atlas, Foundry, and Command binding. `ao-mission governance diff` compares two governance snapshots as readback only. `ao-mission import correlation-evidence` records only a neutral artifact ref and its exact chain role/reference binding; it never applies legacy semantic route, phase, status, blocker, approval, or authority transitions. `ao-mission import atlas-workgraph` records node counts from Atlas workgraphs, refreshes the early-return gate, and routes the first safe ready node to AO Foundry. `ao-mission import atlas-recommendation-readback` records completed Atlas recommendation-wave evidence and closes Mission only when Atlas reports all nodes complete, zero ready nodes, fresh checkpoints, the lease minimum met, and `final_response_allowed=true`; otherwise it keeps Mission routed to AO Atlas with the exact next action. `ao-mission import scheduler-readback` records codex-cron wakeup evidence without granting execution authority, rejects any scheduler readback that claims `executes_work=true`, and classifies evidence freshness. `ao-mission schedule replay` classifies fresh, stale, and unknown scheduler readback fixtures, `ao-mission schedule alerts` turns stale or unknown scheduler readbacks into an attention-required summary, and `ao-mission schedule recover` emits an immediate `ao-mission continue` recommendation when scheduler wakeups are stale or unknown. `ao-mission qualification orchestrate` reads stack qualification orchestration fixtures and reports affected-shard reruns before final full exact-head qualification while requiring shard checkpoints, progress readbacks, global deadlines, and no restart from zero. `ao-mission import scheduler-recovery-readback` and `ao-mission import ledger-compaction-readback` bind those no-authority readbacks into mission evidence and artifact manifests. `ao-mission gateway ledger` persists Telegram and A2A replay intents into `ao.mission.gateway-intent-ledger.v0.1` with no mutation authority, `ao-mission gateway replay-suite` binds Telegram webhook/update replay plus A2A HTTP/lifecycle replay into one no-authority suite readback, `ao-mission gateway readiness-rollup` binds suite, compatibility, archive-validation, and snapshot-diff readbacks into one mission-bound no-authority gateway readiness summary with an optional `correlation_id`, `ao-mission telegram role-matrix` exports allowlisted roles without token values, `ao-mission telegram webhook-replay` gives webhook fixture parity with update replay and classifies fresh, stale, and unknown replay fixture timestamps, `ao-mission a2a serve --http --once` emits an A2A fixture-server readback with Agent Card and JSON-RPC paths but no mutation authority, `ao-mission a2a streaming-denial` records denied streaming/push capability requests, `ao-mission a2a cancellation-replay` records A2A cancel-request/cancelled lifecycle evidence as readback only, `ao-mission a2a compatibility` validates Agent Card, JSON-RPC, task lifecycle, and artifact readback compatibility, and `ao-mission a2a lifecycle` validates cancellation, resume, and artifact lifecycle fixtures as readback only. `ao-mission import foundry-final-rollup` normalizes `completed`, `promoted`, `denied`, and `blocked` terminal statuses. Completed or promoted rollups close only when total node counts agree; denied or blocked rollups become explicit hard blockers. `ao-mission final rollup` includes `final_response_allowed=false` while lease minimums, ready nodes, or exact next actions remain, and it emits at least 10 Feature Depth Recommendations by default. `ao-mission command status` emits a read-only AO Command compatible status packet. `ao-mission artifacts manifest` emits or writes a digest-bound local manifest over mission artifacts without granting execution or approval authority, `ao-mission artifacts validate-manifest` recomputes the manifest and referenced artifact digests so tampering fails closed, and `ao-mission artifacts repair-manifest --out <path>` writes a repaired copy with recomputed text-normalized artifact digests.

...
`ao-mission mission events index` builds a durable local event index over mission records, route decisions, event-loop decisions, and artifacts, while `ao-mission mission events search` emits `ao.mission.event-search-readback.v0.1` without granting execution authority. `ao-mission doctor` emits `ao.mission.doctor-readback.v0.1` with local store, event, and artifact health only.

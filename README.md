...
`ao-mission mission events index` builds a durable local `ao.mission.event-index.v0.2` over mission records, route decisions, event-loop decisions, and artifacts, with `index_digest` and `source_digest` fields so loaded indexes fail closed if tampered. `ao-mission mission events search` emits `ao.mission.event-search-readback.v0.1` without granting execution authority. `ao-mission mission readiness-bundle` binds local readiness summaries from sibling AO repos into one digest-backed readback, `ao-mission gateway replay-bundle` binds Scheduler, Telegram, and A2A replay fixtures into one local no-authority matrix, `ao-mission mission dashboard` emits a compact operator readback over mission status and recent indexed events, and `ao-mission doctor` emits `ao.mission.doctor-readback.v0.1` with local store, event, and artifact health only.
